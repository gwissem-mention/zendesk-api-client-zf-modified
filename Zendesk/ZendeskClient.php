...
class ZendeskClient
{
     ...
     * @return array
     */
    private function _request($page, $method = 'GET', array $jsonData = null, $query = null, $returnObject = false)
    {
        ...
        } elseif ($method == 'GET' && $jsonData) {
        	$params = array();
        	foreach($jsonData as $key => $val) {
        		$params[] = "$key=$val";
        	}
        	$client->setUri($client->getUri(true) . '?' . implode('&', $params));
        }
        ...
        if ($response->isError() && !$returnObject) {
            // Error in response
            return false;
        }
     ...
     * @param array  $jsonData
     */
    public function create($page, array $jsonData, $returnObject = false)
    {
        return $this->_request($page, 'POST', $jsonData, null, $returnObject);
    }
     ...
     * @param string $query
     */
    public function get($page, $query = null, $jsonData = array(), $returnObject = false)
    {
        return $this->_request($page, 'GET', $jsonData, $query, $returnObject);
    }
